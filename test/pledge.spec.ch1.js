...
// Vamos a construir una libreria de promesas similar a ES6, llamada blackBird.js.
// Nuestras promesas van a llamarse "$Promise" para evitar disparar código existente del browser.
// Para concentrarte en los conceptos, blackBird.js va usar muchas variables públicas y no obedecer el standard en su totalidad.

// Para ejecutar los spec, corran `npm test` en este directorio.
// Cuando pases un test, cambien el siguiente test pendiente a activo.
// Este spec es iterativo y opinionado; hagan los tests en orden.
...
--------------------------------------------------------*/
  // Comencemos con una parte esencial y empecemos a definir como se construye la promesa, el argumento `executor`, y resolution / rejection.
  /*========================================================*/

  /* global $Promise */

  // Incluso antes de las clases de ES6, los desarrolladores JS normalmente llamaban ciertas funciones como "clases".
  // A pesar que JS no es un lenguaje basado en clases, todavía tendemos a hablar en terminos de constructores e instancias
  ...
  describe("La clase `$Promise`", function () {
    ...
    // El único argumento de el contructor de una promesa es una función llamada "executor".
    // Vamos a volver a esta función mas adelante.
  ...
  describe("Una instancia de promesa", function () {
    ...
    // Nota -
    // las promesas no estan supuestas a tener un método resolver y rejector público.
    // Sin embargo, esconder estas implementaciones puede ser complicado.
    ...
    describe("resolviendo ", function () {
      it('cambia el estado de la promesa a "fulfilled"', function () {
        // ¿Por qué no "resolved"?
        // Esto va a ser visto en detalle en el Ch. 5, pero por ahora sabe que la terminología de P/A+ estricto establece una distinción entre "resolution" y "fullfillment".
        // Normalmente una promesa "resolved" esta también "fulfilled", pero en un caso particular, una promesa "resolved" puede estar "rejected". Todavía no tenes que saber por qué.
    ...
    describe("rechazando ", function () {
      // Rejection y fulfillment son virtualmente idénticas.
      // esto no deberia requerir mucho mas código
    ...
    describe("una promesa colocada nunca cambia el estado", function () {
      // Si usaste el estado pending para los specs "no afecta un promesa ya completada /rechazada"
      // estos dos specs deberían ya estar pasando.
  ...
  // El constructor Promise toma un argumento (de hecho, ES6 Promise *tienen* que recibir este argumento, o arrojar un error): una función "executor".
  // El executro va a ser llamado con dos argumentos: un "resolver y un "rejector"

  // El executor es una forma para el creador de una nueva promesa de controlar el destino de esa promesa.
  // Recuerda, `._internalResolve` es como nosotros estamos implementando nuestras promesas,
  // pero los usuarios normalmente no pretendemos que tengan acceso a eso directmente.
  // Esto es principalmente para prevenir su abuso:
  // las promises son pretendidas para representar el resultado de una acción asincrónica,
  // pero si *cualquiera* puede llamar `.internalResolve`, no podemos confiar más que la promesa se coloca por el async original.
  // Dado que el executor solo corre cuando la promesa es construida, el acceso al resolver y rejector es naturalmente limitado, haciendo la promesa más confiable.
  ...
  describe("La función executor", function () {
    ...
    describe("argumento resolve", function () {
      // Para este punto podes ser bloqueado por errores como "cannot read X of undefined".
      // Pensa cuidadosamente; podes tener un problema con el *contexto* (el keyword `this`)
      ...
      // No hagas trampa! La función resolver y rejector provistas al executor deberian ser (o llamar) a los metodos resolve y reject internos.
      // Después de todo, trabajaste muy duro para asegurarte `._internalResolve` y `._internalReject` funcionan apropiadamente.
  ...
  // A este punto tenemos unos hechos básicos establecidos.
  // Una promesa empieza con un estado *pending* y ningun valor. En un punto,
  // la promesa puede estar *fulfilled* con data, o *rejected* con una razón. Una vez *colocada* (fulfilled o rejected),
  // una promesa esta trabada en ese estado y no puede ser cambiada.
});
